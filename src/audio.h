...
#define PCM16_OFFSET 24                      /* signed pcm data to signed */
...
#if PDM_OUT
#define OUT_OFFSET 24 /* OUT  offset */
#elif DAC_OUT
...
#define OUT_SCALE 0     /* OUT amplification scale */


static const char *AUDIOTAG = "pdm_mic/out";
...
void example_pdm_mic_init()
{
    ...
    i2s_pin_config_t pin_pdm_config_rx = {
        .data_in_num = GPIO_NUM_34, // ADC1 pins [GPIO 32, GPIO 34, GPIO 36 (VP), GPIO 39 (VN)]
    };
...
void example_pdm_out(void *arg)
{
    ...
    {
        ESP_LOGE(AUDIOTAG, "Partition error: can't find partition name: %s\n", PARTITION_NAME);
        vTaskDelete(NULL);
    }
#if RECORD_IN_FLASH_EN
    //1. Erase flash
